...
class TestObsidianPaths(unittest.TestCase):
    def test_timestamped_path_contains_day_folder_and_run_id(self):
        ...
        path = build_obsidian_note_path("/vault", "AI Digest", dt, "a1b2c3d4e5f6")
    ...
    def test_midnight_boundary_uses_different_day_folders(self):
        ...
        p1 = build_obsidian_note_path("/vault", "AI Digest", one, "run1")
        p2 = build_obsidian_note_path("/vault", "AI Digest", two, "run2")
    ...
    def test_build_path_expands_home_directory(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with patch.dict("os.environ", {"HOME": tmp}, clear=False):
                path = build_obsidian_note_path("~/vault", "AI Digest", dt, "a1")
        self.assertEqual(
            str(path),
            str(Path(tmp) / "vault" / "AI Digest" / "2026-02-22" / "134510-a1.md"),
        )
    ...
    def test_write_obsidian_note_uses_expanded_home_directory(self):
        with tempfile.TemporaryDirectory() as tmp:
            with patch.dict("os.environ", {"HOME": tmp}, clear=False):
                ...
                expected = (
                    Path(tmp) / "vault" / "AI Digest" / "2026-02-22" / "134510-run1.md"
                )
