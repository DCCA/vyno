...
function expectMissing(source, pattern, message) {
...
test("redesign exposes focused navigation surfaces", () => {
  ...
  expectSource(navSource, /label: "Schedule"/, "schedule surface missing")
  ...
  expectSource(navSource, /schedule: "\/schedule"/, "schedule route missing")
...
test("redesign includes responsive nav shell and animation hooks", () => {
  ...
  expectSource(appSource, /border-r border-border bg-card/, "sidebar styling missing")
  expectSource(appSource, /lg:sticky/, "desktop sidebar sticky positioning missing")
  expectSource(appSource, /lg:h-screen/, "desktop sidebar full-height missing")
